...
class LogBase {
public:
    static constexpr const uint32_t MAX_LOG_LEN = 16000;
    static constexpr const uint32_t MSG_HDR_LEN = 200;
    ...
    do {                                                                                                        \
        if (AlogCheckDebugLevel(static_cast<int>(MOD_ID), (LOG_LEVEL)) == 1) {                                  \
            AlogRecord(static_cast<int>(MOD_ID), DLOG_TYPE_DEBUG, (LOG_LEVEL),                                  \
                ...
                (OPS_UTILS_LOG_PACKAGE_TYPE), __FUNCTION__, AiInfraOps::utils::LogBase::GetTid(),                      \
                AiInfraOps::utils::LogBase::GetStr(AiInfraOps::utils::LogBase::GetOpInfo(OPS_DESC)), ##__VA_ARGS__);   \
        }                                                                                                       \
    }while (0)
...
#define OPS_LOG_STUB_FULL(LEVEL, OPS_DESC, FMT, ...)                                                                   \
  do {                                                                                                                 \
    ...
    char msgbufxyz[AiInfraOps::utils::LogBase::MAX_LOG_LEN];                                                           \
    size_t msgmaxlen = (MSG_LENGTH - AiInfraOps::utils::LogBase::MSG_HDR_LEN);                                         \
